...
pub struct MessageDescriptor {
    ...
    pub table_version: Option<u16>,
    pub generating_process_identifier: Option<u16>,
...
impl GribEngine {
    ...
    pub fn decode_bytes_message(
        ...
        self.decode_message(&data[start..end], descriptor)
    }
    ...
    pub fn decode_message(
        ...
        match descriptor.edition {
            Some(GribEdition::Grib2) => decode_grib2_message(raw_message, descriptor),
            Some(GribEdition::Grib1) => decode_grib1_message(raw_message, descriptor),
...
fn parse_idx_line(line: &str, fallback_message_no: u64) -> Result<MessageDescriptor, String> {
    ...
    Ok(MessageDescriptor {
        ...
        subcenter: None,
        table_version: None,
        generating_process_identifier: None,
...
fn scan_grib1_message(
    ...
    let descriptor = MessageDescriptor {
        ...
        center: Some(product.center as u16),
        subcenter: product.subcenter.map(u16::from),
        table_version: Some(product.table_version as u16),
        generating_process_identifier: Some(product.generating_process_identifier as u16),
        ...
        scan_mode: grid.map(|item| item.scan_mode),
...
fn parse_grib1_pds(section: &[u8]) -> Result<Grib1ProductMetadata, String> {
    ...
    Ok(Grib1ProductMetadata {
        ...
        center: section[4],
        subcenter: section.get(25).copied(),
        generating_process_identifier: section[5],
        has_gds: flag & 0x80 != 0,
        has_bms: flag & 0x40 != 0,
        parameter: section[8],
        level_type: section[9],
        level_value: match section[9] {
            112 => section[10] as u16,
            _ => read_u16_be(section, 10)?,
        },
...
fn parse_grib1_gds(section: &[u8]) -> Result<GridSectionMetadata, String> {
    ...
    match data_representation_type {
        0 => {
        ...
        3 => {
            ...
            Ok(GridSectionMetadata {
                ...
                lat2: None,
                lon2: None,
                dx: Some(read_u24_be(section, 20)? as f64),
                dy: Some(read_u24_be(section, 23)? as f64),
                gaussian_n: None,
            })
        }
        4 => {
            ...
            let gaussian_n = read_u16_be(section, 25)? as usize;
            let reduced = nx == u16::MAX as usize || read_u16_be(section, 23)? == u16::MAX;
            let point_count = if reduced {
                let mut count = 0usize;
                for chunk in section[32..].chunks_exact(2).take(ny) {
                    count += u16::from_be_bytes([chunk[0], chunk[1]]) as usize;
                }
                Some(count)
            } else {
                None
            ...
            Ok(GridSectionMetadata {
                template: if reduced { 41 } else { 40 },
                nx,
                ny,
                point_count,
                scan_mode: section[27],
                ...
                dx: Some(read_u16_be(section, 23)? as f64 / 1000.0),
                dy: None,
                gaussian_n: Some(gaussian_n),
            })
        }
        50 | 51 => Ok(GridSectionMetadata {
            template: data_representation_type as u16,
            nx: read_u16_be(section, 6)? as usize,
            ny: read_u16_be(section, 8)? as usize,
            point_count: None,
            scan_mode: 0,
            projection: ProjectionKind::LatLon,
            lat1: None,
...
fn scan_grib2_message(
    ...
    let mut product_template = None;
    let mut data_template = None;
    let mut bits_per_value = None;
    let mut generating_process_identifier = None;
    ...
    while cursor + 5 <= message.len() {
        ...
        match section_number {
            1 => {
                let section1 = parse_section1_metadata(section);
                reference_time = section1.reference_time;
                center = section1.center;
                subcenter = section1.subcenter;
            }
            3 => {
                let grid = parse_section3_metadata(section)?;
                grid_template = Some(grid.template);
                scan_mode = Some(grid.scan_mode);
                nx = Some(grid.nx as u32);
                ny = Some(grid.ny as u32);
            }
            4 => {
                let product = parse_section4_product(section)?;
                product_template = Some(product.template);
                category = Some(product.category);
                parameter_number = Some(product.number);
                generating_process_identifier = Some(product.generating_process_identifier);
                forecast_time_value = Some(product.forecast_time);
                forecast_time_unit = Some(time_unit_name(product.time_unit).to_string());
                level_type = Some(product.level_type);
            ...
            5 => {
                let data = parse_section5_metadata(section)?;
                data_template = Some(data.template);
                bits_per_value = Some(data.bits_per_value);
            }
            _ => {}
        }
    ...
    let descriptor = MessageDescriptor {
        ...
        subcenter,
        table_version: None,
        generating_process_identifier: generating_process_identifier.map(u16::from),
        reference,
        variable: short_name.to_string(),
        parameter_name: Some(parameter_name.to_string()),
        units: Some(units.to_string()),
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct Section1Metadata {
    reference_time: Option<String>,
    center: Option<u16>,
    subcenter: Option<u16>,
}

fn parse_section1_metadata(section: &[u8]) -> Section1Metadata {
    if section.len() < 19 {
        return Section1Metadata {
            reference_time: None,
            center: None,
            subcenter: None,
        };
    ...
    let month = section.get(14).copied();
    let day = section.get(15).copied();
    let hour = section.get(16).copied();
    let minute = section.get(17).copied();
    let second = section.get(18).copied();
    let reference_time = match (year, month, day, hour, minute, second) {
        (Some(year), Some(month), Some(day), Some(hour), Some(minute), Some(second)) => Some(
    ...
    Section1Metadata {
        reference_time,
        center: read_u16_be(section, 5).ok(),
        subcenter: read_u16_be(section, 7).ok(),
    }
}
...
fn parse_section3_metadata(section: &[u8]) -> Result<GridSectionMetadata, String> {
    ...
    match template {
        ...
        40 => {
            if section.len() < 72 {
                return Err("section 3 template 40 too short".to_string());
            }
            let basic_angle = read_u32_be(section, 38)?;
            let subdivisions = read_u32_be(section, 42)?;
            let divisor = if basic_angle == 0 || subdivisions == 0 {
            ...
            let gaussian_n = read_u32_be(section, 67)? as usize;
            Ok(GridSectionMetadata {
                template,
                nx,
                ny,
...
fn parse_section4_product(section: &[u8]) -> Result<ProductMetadata, String> {
    ...
    let template = read_u16_be(section, 7)?;
    let category = section[9];
    let number = section[10];
    let generating_process_identifier = section.get(13).copied().unwrap_or(0);
...
fn decode_grib1_message(
    message: &[u8],
    mut descriptor: MessageDescriptor,
) -> Result<DecodedField, String> {
    ...
    let expected_points = grid.point_count.unwrap_or_else(|| grid.nx * grid.ny);
    ...
    let bitmap = if product.has_bms {
        ...
        let parsed = parse_grib1_bms(&message[cursor..cursor + bms_length], expected_points)?;
        cursor += bms_length;
        Some(parsed)
    } else {
        None
    ...
    descriptor.data_template = Some(0);
    descriptor.bits_per_value = Some(binary.bits_per_value);
    ...
    let flattened_storage = grid.point_count.is_some() || matches!(grid.template, 50 | 51);
    if !flattened_storage && values.len() != grid.nx * grid.ny {
        return Err(format!(
    ...
    if let Some(point_count) = grid.point_count {
        if values.len() != point_count {
            return Err(format!(
                "decoded GRIB1 value count {} does not match expected point count {}",
                values.len(),
                point_count
            ));
        }
    ...
    let storage_nx = if flattened_storage {
        values.len()
    } else {
        grid.nx
    };
    let storage_ny = if flattened_storage { 1 } else { grid.ny };
    let grid_spec = GridSpec {
        ...
        dx_m: match grid.projection {
            ProjectionKind::LambertConformal
            | ProjectionKind::Mercator
            | ProjectionKind::PolarStereographic => grid.dx,
            _ => None,
        },
        dy_m: match grid.projection {
            ProjectionKind::LambertConformal
            | ProjectionKind::Mercator
            | ProjectionKind::PolarStereographic => grid.dy,
            _ => None,
        },
    };
    let (x_axis, y_axis) = if flattened_storage {
        (None, None)
    } else {
        build_axes(&grid)
    };
    Ok(DecodedField {
        descriptor,
        grid: Grid2D::new(storage_nx, storage_ny, values),
        grid_spec,
        x_axis,
        y_axis,
        missing_count,
...
fn decode_grib2_message(
    ...
    let mut grid = grid.ok_or_else(|| "GRIB2 message missing section 3".to_string())?;
    let data_representation =
        data_representation.ok_or_else(|| "GRIB2 message missing section 5".to_string())?;
    descriptor.data_template = Some(data_representation.template);
    descriptor.bits_per_value = Some(data_representation.bits_per_value);
    let data_section = data_section.ok_or_else(|| "GRIB2 message missing section 7".to_string())?;

    if grid.template == 40
        && data_representation.num_points > 0
        && grid.nx > 0
        && data_representation.num_points != grid.nx * grid.ny
        && data_representation.num_points % grid.nx == 0
    {
        grid.ny = data_representation.num_points / grid.nx;
    }
    ...
    let mut missing_count =
        if data_representation.template <= 3 && data_representation.missing_value_management != 0 {
    ...
    if let Some(bitmap) = bitmap.as_ref() {
        missing_count += bitmap.iter().filter(|is_present| !**is_present).count();
        values = apply_bitmap(values, bitmap);
    }
...
fn unpack_simple(
    ...
    let scaling = ValueScaling::new(metadata);
    if metadata.bits_per_value == 0 {
        return Ok(vec![scaling.scale_u64(0); num_points]);
    }

    let mut reader = BitReader::new(data);
    let mut values = Vec::with_capacity(num_points);
    for _ in 0..num_points {
        values.push(scaling.scale_u64(reader.read_bits(metadata.bits_per_value as usize)?));
    }
    Ok(values)
}
...
fn unpack_complex(
    ...
    if metadata.missing_value_management == 0 {
        return unpack_complex_no_missing(data, metadata, num_points);
    }
...
fn unpack_complex_no_missing(
    ...
    match spatial.as_ref().map(|spatial| spatial.order) {
        Some(1) => {
            let spatial = spatial
                .as_ref()
                .ok_or_else(|| "missing spatial differencing metadata".to_string())?;
            let first = *spatial.initial_values.first().ok_or_else(|| {
                "missing first initial value for order-1 differencing".to_string()
            })?;
            let mut previous = first;
            let mut seed_points_to_skip = num_points.min(1);

            if seed_points_to_skip == 1 {
                values.push(scaling.scale_i64(first));
            }

            for group_index in 0..metadata.number_of_groups {
                let group_reference = group_references[group_index] as i64;
                let group_width = group_widths[group_index] as usize;
                let mut group_length = group_lengths[group_index] as usize;
                let delta_base = group_reference + spatial.minimum;

                if seed_points_to_skip > 0 && group_length > 0 {
                    if group_width != 0 {
                        let _ = reader.read_bits(group_width)?;
                    }
                    seed_points_to_skip -= 1;
                    group_length -= 1;
                }
                ...
                if group_width == 0 {
                    for _ in 0..group_length {
                        previous += delta_base;
                        values.push(scaling.scale_i64(previous));
                    }
                    continue;
                }

                for _ in 0..group_length {
                    previous += delta_base + reader.read_bits(group_width)? as i64;
                    values.push(scaling.scale_i64(previous));
                }
            }
        }
        Some(2) => {
            ...
            let mut seed_points_to_skip = num_points.min(2);

            if seed_points_to_skip >= 1 {
                values.push(scaling.scale_i64(first));
            }
            if seed_points_to_skip == 2 {
                values.push(scaling.scale_i64(second));
            }
            ...
            for group_index in 0..metadata.number_of_groups {
                ...
                while seed_points_to_skip > 0 && group_length > 0 {
                    if group_width != 0 {
                        let _ = reader.read_bits(group_width)?;
                    }
                    seed_points_to_skip -= 1;
                ...
                if group_width == 0 {
                    for _ in 0..group_length {
                        previous_first_difference += delta_base;
                        previous_value += previous_first_difference;
                        values.push(scaling.scale_i64(previous_value));
                    }
                    continue;
                }

                for _ in 0..group_length {
                    previous_first_difference += delta_base + reader.read_bits(group_width)? as i64;
                    previous_value += previous_first_difference;
                    values.push(scaling.scale_i64(previous_value));
                }
            }
        ...
        Some(order) => {
            return Err(format!("unsupported spatial differencing order {order}"));
        }
        None => {
            for group_index in 0..metadata.number_of_groups {
                let group_reference = group_references[group_index] as i64;
                ...
                if group_width == 0 {
                    let value = scaling.scale_i64(group_reference);
                    values.resize(values.len() + group_length, value);
                    continue;
                }

                for _ in 0..group_length {
                    let raw = group_reference + reader.read_bits(group_width)? as i64;
                    values.push(scaling.scale_i64(raw));
                }
            }
        }
    }
...
impl ValueScaling {
    fn new(metadata: DataRepresentationMetadata) -> Self {
        Self {
            reference: metadata.reference_value as f64,
            binary_scale: 2.0_f64.powi(metadata.binary_scale as i32),
            decimal_scale: 10.0_f64.powi(-(metadata.decimal_scale as i32)),
        }
    }

    fn scale_u64(self, raw: u64) -> f64 {
        self.scale_i64(raw as i64)
    }

    fn scale_i64(self, raw: i64) -> f64 {
        (self.reference + (raw as f64) * self.binary_scale) * self.decimal_scale
    }
}
...
fn apply_scaling(raw: &[u64], metadata: DataRepresentationMetadata) -> Vec<f64> {
    ...
    for &value in raw {
        values.push(scaling.scale_u64(value));
    }
    values
}
...
fn apply_scaling_with_missing(
    ...
    for value in raw {
        values.push(match value {
            Some(value) => scaling.scale_i64(*value),
            None => f64::NAN,
        });
    }
    values
    ...
    match grid.projection {
        ProjectionKind::LatLon => match (grid.lon1, grid.lon2, grid.lat1, grid.lat2) {
            (Some(lon1), Some(lon2), Some(lat1), Some(lat2)) => {
                let scans_i_negatively = grid.scan_mode & 0x80 != 0;
                let (x_start, x_end) = unwrap_longitude_range(lon1, lon2, scans_i_negatively);
                let x_values = if grid.nx <= 1 {
                    vec![x_start]
                } else if let Some(dx) = grid.dx {
                    let step = if scans_i_negatively { -dx } else { dx };
                    (0..grid.nx)
                        .map(|idx| x_start + step * idx as f64)
                        .collect()
                } else {
                    let step = (x_end - x_start) / (grid.nx as f64 - 1.0);
                    (0..grid.nx)
                        .map(|idx| x_start + step * idx as f64)
                        .collect()
                };
                let scans_j_positively = grid.scan_mode & 0x40 != 0;
                let mut y_values = if let Some(gaussian_n) = grid.gaussian_n {
                    let north = lat1.max(lat2);
                    let south = lat1.min(lat2);
                    let mut values = gaussian_axis_values(grid.ny, gaussian_n, north, south);
                    orient_axis_to_first_point(&mut values, lat1);
                    values
                } else if grid.ny <= 1 {
                    vec![lat1]
                } else if let Some(dy) = grid.dy {
                    let step = if scans_j_positively { dy } else { -dy };
                    (0..grid.ny).map(|idx| lat1 + step * idx as f64).collect()
                } else {
                    let step = (lat2 - lat1) / (grid.ny as f64 - 1.0);
                    (0..grid.ny).map(|idx| lat1 + step * idx as f64).collect()
                };
                if grid.gaussian_n.is_none() {
                    orient_axis_to_first_point(&mut y_values, lat1);
                }
                (
                    Some(CoordinateAxis {
...
fn unwrap_longitude_range(start: f64, end: f64, scans_negatively: bool) -> (f64, f64) {
    let mut start = start;
    let mut end = end;
    if !scans_negatively && start >= 180.0 && end < start {
        start -= 360.0;
    }
    if scans_negatively {
        while end > start {
...
fn orient_axis_to_first_point(values: &mut [f64], first_point: f64) {
    if values.len() <= 1 {
        return;
    }
    let first_delta = (values[0] - first_point).abs();
    let last_delta = (values[values.len() - 1] - first_point).abs();
    if last_delta < first_delta {
        values.reverse();
    }
}

fn gaussian_axis_values(ny: usize, gaussian_n: usize, north: f64, south: f64) -> Vec<f64> {
    let full = gaussian_latitudes(gaussian_n);
    if full.len() == ny {
        return full;
    }
    if full.len() > ny {
        let north_idx = full
            ...
            .min_by(|(_, left), (_, right)| {
                ((*left - north).abs()).total_cmp(&(*right - north).abs())
            })
            .map(|(idx, _)| idx)
            .unwrap_or(0);
        let south_idx = full
            ...
            .enumerate()
            .min_by(|(_, left), (_, right)| {
                ((*left - south).abs()).total_cmp(&(*right - south).abs())
            })
            .map(|(idx, _)| idx)
            .unwrap_or(full.len().saturating_sub(1));
        let start = north_idx.min(south_idx);
        let mut end = start + ny;
        if end > full.len() {
            end = full.len();
        }
        let start = end.saturating_sub(ny);
        return full[start..end].to_vec();
    }
    if ny <= 1 {
        vec![north]
    } else {
        let step = (south - north) / (ny as f64 - 1.0);
        (0..ny).map(|idx| north + step * idx as f64).collect()
    }
}

fn gaussian_latitudes(gaussian_n: usize) -> Vec<f64> {
    let total = gaussian_n * 2;
    let mut roots = vec![0.0; total];
    let midpoint = total.div_ceil(2);
    for i in 0..midpoint {
        let mut z = (std::f64::consts::PI * (i as f64 + 0.75) / (total as f64 + 0.5)).cos();
        loop {
            let mut p1 = 1.0;
            let mut p2 = 0.0;
            for j in 1..=total {
            ...
            let derivative = total as f64 * (z * p1 - p2) / (z * z - 1.0);
            let next = z - p1 / derivative;
            if (next - z).abs() < 1.0e-14 {
                z = next;
                break;
            }
            z = next;
    ...
    roots
        .into_iter()
        .rev()
        .map(|value| value.asin().to_degrees())
        .collect()
}
...
impl<'a> BitReader<'a> {
    ...
    fn read_bits(&mut self, count: usize) -> Result<u64, String> {
        ...
        while self.bits_in_buffer < count {
            let Some(&next_byte) = self.data.get(self.byte_pos) else {
                return Err("not enough packed bits remaining".to_string());
            };
            self.bit_buffer = (self.bit_buffer << 8) | next_byte as u128;
            self.bits_in_buffer += 8;
            self.byte_pos += 1;
        }

        let trailing_bits = self.bits_in_buffer - count;
        let mask = if count == 64 {
            u64::MAX as u128
        } else {
            (1u128 << count) - 1
        };
        let out = ((self.bit_buffer >> trailing_bits) & mask) as u64;
        self.bits_in_buffer = trailing_bits;
        if trailing_bits == 0 {
            self.bit_buffer = 0;
        } else {
            self.bit_buffer &= (1u128 << trailing_bits) - 1;
        }
        Ok(out)
    }
    ...
    fn align_to_octet(&mut self) {
        ...
        if remainder != 0 {
            ...
            if self.bits_in_buffer == 0 {
                ...
                self.bit_buffer &= (1u128 << self.bits_in_buffer) - 1;
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn bit_reader_reads_unaligned_fields_across_octets() {
        let mut reader = BitReader::new(&[0b1011_0110, 0b0110_0011, 0b1111_0000]);
        assert_eq!(reader.read_bits(3).unwrap(), 0b101);
        assert_eq!(reader.read_bits(5).unwrap(), 0b10110);
        assert_eq!(reader.read_bits(6).unwrap(), 0b011000);
        assert_eq!(reader.read_bits(10).unwrap(), 0b1111110000);
    }

    #[test]
    fn bit_reader_supports_unaligned_64_bit_reads() {
        let mut reader = BitReader::new(&[0xAB, 0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0]);
        assert_eq!(reader.read_bits(4).unwrap(), 0xA);
        assert_eq!(reader.read_bits(64).unwrap(), 0xB123_4567_89AB_CDEF);
    }

    #[test]
    fn bit_reader_align_to_octet_skips_partial_byte() {
        let mut reader = BitReader::new(&[0b1011_0110, 0b0110_0011, 0b1111_0000]);
        assert_eq!(reader.read_bits(3).unwrap(), 0b101);
        reader.align_to_octet();
        assert_eq!(reader.read_bits(8).unwrap(), 0b0110_0011);
